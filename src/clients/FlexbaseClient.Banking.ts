...
export class FlexbaseClientBanking extends FlexbaseClientBase {
  ...
  async getBankingStatementPdf(companyId: string, statementId: string, options?: BankingParameters): Promise<SingleStatementResponse | ArrayBuffer> {
    try {
      const params = this.bankingParams(options);

      if (options?.isPdf) {
        return await this.client.url(`/banking/${companyId}/statements/${statementId}`).query(params).get().arrayBuffer();
      }

      const response = await this.client.url(`/banking/${companyId}/statements/${statementId}`).query(params).get().json<SingleStatementResponse>();
